...
def fetch_datagolf_odds_with_event(market_key: str) -> tuple | None:
    """
    Like fetch_datagolf_odds but also returns the event name from the response.
    Returns (event_name, {player: odds}) or None if unavailable.
    """
    url = (
        f"{DATAGOLF_BASE}/betting-tools/outrights"
        f"?tour=pga&market={market_key}&odds_format=american"
    ...
    try:
        resp = requests.get(url, timeout=15)
        if resp.status_code in (403, 401):
            print(f"  [DataGolf] Auth error {resp.status_code}")
            return None
        if resp.status_code == 404:
            return None
        resp.raise_for_status()
...
def run_datagolf_snapshot(sport_key: str, event_name: str):
    ...
    for label, dg_market in DATAGOLF_MARKETS.items():
        try:
            result = fetch_datagolf_odds_with_event(dg_market)
            if result is None:
                print(f"    [{label}] not available yet")
                continue
            dg_event_name, odds = result
            # Verify DataGolf is serving odds for the right event
            # If they are still showing last week's event, skip and wait
            dg_slug   = dg_event_name.lower().replace(" ", "")
            want_slug = event_name.lower().replace(" ", "")
            if dg_slug != want_slug:
                print(f"    [{label}] DataGolf showing '{dg_event_name}' not '{event_name}' yet — skipping")
                continue
            if not odds:
    ...
    if any_saved:
        ...
        print(f"[DataGolf] Waiting for {event_name} odds to post (DataGolf may still be on last week)")
