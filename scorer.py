...
def get_wednesday_cutoff(start_date: str) -> str:
    """
    Given a tournament start_date (YYYY-MM-DD, Thursday),
    return ISO timestamp for Wednesday 11:59 PM PST (= Thursday 07:59 UTC).
    Closing line is locked at this time — no tournament-day odds.
    """
    try:
        thu = datetime.strptime(start_date[:10], "%Y-%m-%d").replace(tzinfo=timezone.utc)
        # Wednesday = thu - 1 day, 07:59 UTC = 11:59 PM PST (UTC-8 winter)
        wed_cutoff = thu - timedelta(seconds=1)  # just before Thursday midnight UTC
        return wed_cutoff.isoformat()
    except Exception:
        return ""


def get_latest_implied(cur, sport_key, player, market, cutoff_ts: str = None):
    if cutoff_ts:
        cur.execute("""
            SELECT implied_pct FROM snapshots
            WHERE sport_key=? AND player=? AND market=? AND ts <= ?
            ORDER BY ts DESC LIMIT 1
        """, (sport_key, player, market, cutoff_ts))
    else:
        cur.execute("""
            SELECT implied_pct FROM snapshots
...
def get_latest_american(cur, sport_key, player, market, cutoff_ts: str = None):
    if cutoff_ts:
        cur.execute("""
...
def calculate_scores(sport_key: str = None) -> list[PlayerScore]:
    ...
    if sport_key is None:
        cur.execute("SELECT sport_key, title, start_date FROM events ORDER BY last_seen DESC LIMIT 1")
        row = cur.fetchone()
        if not row:
            con.close()
            return []
        sport_key, event_title, start_date = row
    else:
    ...
    # If tournament has started (start_date is in the past), lock closing
    # line at Wednesday 11:59 PM PST to avoid using live in-play odds
    cutoff_ts = None
    if start_date:
        try:
            thu = datetime.strptime(start_date[:10], "%Y-%m-%d").replace(tzinfo=timezone.utc)
            if datetime.now(timezone.utc) >= thu:
                cutoff_ts = get_wednesday_cutoff(start_date)
        except Exception:
            pass
    ...
    for player in players:
        ...
        w_cur    = get_latest_implied(cur, sport_key, player, "winner",  cutoff_ts)
        t5_cur   = get_latest_implied(cur, sport_key, player, "top5",    cutoff_ts)
        t10_cur  = get_latest_implied(cur, sport_key, player, "top10",   cutoff_ts)
        ...
        w_am_cur     = get_latest_american(cur, sport_key, player, "winner",  cutoff_ts) or 0
        t5_am_cur    = get_latest_american(cur, sport_key, player, "top5",    cutoff_ts) or 0
        t10_am_cur   = get_latest_american(cur, sport_key, player, "top10",   cutoff_ts) or 0
...
def get_all_events() -> list[dict]:
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    # Only return datagolf_ events — exclude old Odds API golf_ keys (majors futures)
    cur.execute("""
        SELECT sport_key, title, first_seen, last_seen, start_date
        FROM events
        WHERE sport_key LIKE 'datagolf_%'
        ORDER BY COALESCE(start_date, last_seen) DESC
    """)
    rows = cur.fetchall()
    con.close()
    return [{"key": r[0], "title": r[1], "first_seen": r[2], "last_seen": r[3], "start_date": r[4]}
            for r in rows]
