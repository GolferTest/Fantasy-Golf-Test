...
class PlayerScore:
    ...
    sport_key:      str
    event_title:    str
    # Composite columns (sum of available markets)
    ...
    delta:          float  # current_total - baseline
    # Per-market columns
    winner_open:    float
...
def calculate_scores(sport_key: str = None) -> list[PlayerScore]:
    ...
    for player in players:
        ...
        # Composite baseline/current/delta — sum only available markets
        t5_open_v   = t5_open   if t5_open   is not None else 0.0
        t5_cur_v    = t5_cur    if t5_cur    is not None else 0.0
        t10_open_v  = t10_open  if t10_open  is not None else 0.0
        t10_cur_v   = t10_cur   if t10_cur   is not None else 0.0
