...
def calc_payout(finish_pos: int | None, american_odds: int, bet: float = 25.0) -> float:
    """
    Calculate P&L for a $25 bet.
    We bet on winner market. Player pays out if they finish:
      1st  → winner odds pay
      else → -$25
    """
    if finish_pos is None:
        return -bet  # MC/WD = loss
    if finish_pos == 1:
        if american_odds > 0:
...
def get_event_tracker(sport_key: str) -> dict:
    ...
    for p in picks:
        ...
        pl         = calc_payout(pos, p.winner_open_american) if finish_str else None
        if pl is not None:
            event_pl += pl
        pick_rows.append({
            "player":               p.player,
    ...
    return {
        ...
        "picks_settled":   all(r["pl"] is not None for r in pick_rows),
    }
