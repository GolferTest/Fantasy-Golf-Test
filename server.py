...
def _run_scheduler():
    ...
    def get_current_event_key() -> str | None:
        """
        Find the tournament starting within the next 8 days OR currently in progress.

        Logic:
          1. Check DB for events with commence_time within 0-8 days → that's this week
          2. Check for in-progress events (started 0-4 days ago)
          3. If nothing within 8 days → return None (still waiting for new event to appear)

        Deliberately excludes events >8 days away (like Masters when Valero is this week)
        so we don't accidentally hunt the wrong tournament.
        """
        ...
        # In-progress: started in last 4 days
        in_progress = [(k, d) for k, d in scored if -4 <= d <= 0]
        if in_progress:
            chosen = sorted(in_progress, key=lambda x: x[1], reverse=True)[0][0]
            return chosen

        # Upcoming within 8 days — this week's event
        this_week = [(k, d) for k, d in scored if 0 < d <= 8]
        if this_week:
            chosen = sorted(this_week, key=lambda x: x[1])[0][0]
            return chosen

        # Nothing within 8 days → no target yet, keep watching for new events
        return None
    ...
    def smart_snapshot():
        ...
        if still_hunting:
            ...
            if sport_key is None:
                ...
                print(f"[Phase 1 Hunting] Target: {sport_key} | Missing markets: {still_hunting}")
                focused_snapshot_job()
                _last_pull_time[0] = now_local
        else:
