...
def _run_scheduler():
    ...
    def refresh_commence_times():
        """
        Discover new events and fill in missing commence_times.
        Checks DataGolf schedule first (covers all weekly events),
        then The Odds API (majors fallback).
        """
        from tracker import (fetch_active_pga_events, fetch_datagolf_current_event,
                              get_datagolf_sport_key, upsert_event)
        from datetime import timezone
        import requests as _req
        ts = datetime.now(timezone.utc).isoformat()

        # Step 1: Check DataGolf for this week's event and upsert it
        try:
            dg_event = fetch_datagolf_current_event()
            if dg_event:
                event_name = (dg_event.get("event_name") or
                              dg_event.get("name") or
                              dg_event.get("tournament_name") or "Unknown")
                start_date = dg_event.get("date") or dg_event.get("start_date") or ""
                sport_key  = get_datagolf_sport_key(event_name, start_date)
                # Build an ISO commence_time from the date
                commence_time = f"{start_date}T12:00:00+00:00" if start_date else None
                con = sqlite3.connect(DB_PATH)
                upsert_event(con, sport_key, event_name, ts, commence_time)
                con.commit()
                con.close()
                print(f"  [refresh] DataGolf event: {event_name} ({sport_key})")
        except Exception as e:
            print(f"  [refresh] DataGolf discovery error: {e}")

        # Step 2: Discover majors from The Odds API
        try:
            ...
            print(f"  [refresh] Odds API discovery error: {e}")

        # Step 3: Fill in missing commence_times for Odds API events
        try:
            con = sqlite3.connect(DB_PATH)
            cur = con.cursor()
            cur.execute("""
                SELECT sport_key FROM events
                WHERE commence_time IS NULL AND sport_key NOT LIKE 'datagolf_%'
            """)
            ...
            print(f"  [refresh] commence_time fill error: {e}")
    ...
    def focused_snapshot_job():
        """
        Pull current week's tournament odds.
        Primary source: DataGolf (covers all weekly PGA Tour events).
        Fallback: The Odds API (majors only, if DataGolf has no event this week).
        """
        from tracker import (fetch_datagolf_current_event, run_datagolf_snapshot,
                              get_datagolf_sport_key, fetch_odds_for_event,
                              save_snapshot, upsert_event, MARKETS)
        from datetime import timezone
        ...
        if dg_event:
            event_name = (dg_event.get("event_name") or
                          dg_event.get("name") or
                          dg_event.get("tournament_name") or "Unknown Event")
            start_date = (dg_event.get("date") or
                          dg_event.get("start_date") or "")
            sport_key = get_datagolf_sport_key(event_name, start_date)
            print(f"\n[DataGolf primary] {event_name} → {sport_key}")
            run_datagolf_snapshot(sport_key, event_name)
            return

        # ── Fallback: The Odds API for majors ───────────────────────────────
        print("[DataGolf] No current event found — falling back to Odds API (majors)")
        sport_keys = get_current_event_keys()
        if not sport_keys:
            print("[Scheduler] No active events found in either source — skipping")
            return
        ts = datetime.now(timezone.utc).isoformat()
        for sport_key in sport_keys:
            ...
            print(f"\n[Odds API fallback] {title} ({sport_key})")
    ...
    def smart_snapshot():
        ...
        if still_hunting:
            ...
            # Phase 2: all 3 captured — pull every 50 min to track movement
            mins_since = (now_local - _last_pull_time[0]).total_seconds() / 60
            if mins_since >= 50:
