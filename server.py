...
def _run_scheduler():
    ...
    # ── Helper: identify this week's tournament ───────────────────────────
    def get_current_event_key() -> str | None:
        """
        Find the single tournament to focus on this week.
        Strategy: from all events in our DB, pick the one whose commence_time
        is soonest in the future OR started within the last 4 days (in progress).
        Falls back to querying The Odds API events endpoint for commence_time
        if not stored locally yet.
        Returns sport_key string or None.
        """
        ...
        try:
            con = sqlite3.connect(DB_PATH)
            cur = con.cursor()
            cur.execute("SELECT sport_key, title, commence_time FROM events")
            rows = cur.fetchall()
            con.close()
        except Exception:
            return None
        ...
        scored = []
        for sport_key, title, commence_time in rows:
            if not commence_time:
                # Try to fetch it from The Odds API events endpoint (free, no quota cost)
                try:
            ...
            if commence_time:
                try:
                    ...
                    scored.append((sport_key, days_from_now))
                except Exception:
                    pass

        if not scored:
            # No commence_time data — fall back to most recently seen event
            try:
                con = sqlite3.connect(DB_PATH)
                cur = con.cursor()
        ...
        # In-progress: started 0-4 days ago → pick the most recently started
        in_progress = [(k, d) for k, d in scored if -4 <= d <= 0]
        if in_progress:
            return sorted(in_progress, key=lambda x: x[1], reverse=True)[0][0]

        # Upcoming: pick the soonest one
        upcoming = [(k, d) for k, d in scored if d > 0]
        if upcoming:
            return sorted(upcoming, key=lambda x: x[1])[0][0]

        # All in the past — pick closest to now
        return sorted(scored, key=lambda x: abs(x[1]))[0][0]

    def focused_snapshot_job():
        """Pull only the current week's tournament instead of all 4."""
        sport_key = get_current_event_key()
        if not sport_key:
            print("[Scheduler] Could not identify current event — skipping pull")
            return
    ...
    def get_captured_markets() -> set:
        """
        Return which markets already have an opening line for THIS week's event.
        Each market hunts independently — winner may be captured while T5/T10 still hunt.
        """
        try:
            sport_key = get_current_event_key()
            if not sport_key:
    ...
    def smart_snapshot():
        ...
        if still_hunting:
            # Phase 1: hunt every 5 min — costs nothing until lines post (422 = free)
