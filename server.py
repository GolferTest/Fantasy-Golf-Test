...
# ── Run scheduler in background thread so one Railway process does everything ──
import schedule as _schedule
import pytz as _pytz

def _start_scheduler():
    """Run the snapshot scheduler in a background thread."""
    import time
    from tracker import run_snapshot
    from scheduler import snapshot_job, wednesday_picks_job, staggered_results_job

    LOCAL_TZ = _pytz.timezone("America/Los_Angeles")

    # Run first snapshot immediately on startup
    try:
        run_snapshot()
    ...
    _schedule.every(2).hours.do(snapshot_job)

    # Wednesday 2 PM Pacific picks email
    _schedule.every().wednesday.at("21:00").do(wednesday_picks_job)

    # Staggered results batches
    _schedule.every().tuesday.at("01:00").do(lambda: staggered_results_job(1))
    _schedule.every().wednesday.at("01:00").do(lambda: staggered_results_job(2))
    _schedule.every().wednesday.at("17:00").do(lambda: staggered_results_job(3))

    print("Background scheduler running — pulling every 2 hours.")
    while True:
